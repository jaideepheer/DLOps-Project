...
def valid_epoch(model, criterion, optimizer, valid_dl, valid_ds, example_ct = 0):
    #validation
    model.eval()
    running_loss = 0.0
    running_corrects = 0
    ...
    with torch.no_grad():
        for x,y in valid_dl:
            ...
            # Test Accuracy Logs
            if args.wandb:
                wandb.log({"running_val_accuracy": running_corrects / total}, step=example_ct)
                example_ct += y.size(0)
            ########################################################################
...
def train(model, criterion, optimizer, train_dl, valid_dl, valid_ds):
    ...
    for epoch in range(args.epochs):
        ...
        accuracy = valid_epoch(model, criterion, optimizer, valid_dl, valid_ds, example_ct)
