...
class ImageDataset(Dataset):
    def __init__(self, x, y, ds_type):
        ...
        if args.ds_in_memory_cache is True:
            self.load_file = lru_cache(maxsize=None)(self.load_file)
...
def data_manipulation(path, mode=args.mode):
    ...
    if mode == "train":
        ...
        df_fnames = (
            train_df["image_id"].append(test_df["image_id"], ignore_index=True).tolist()
        )
    else:
        return train_df, vocab

    filepath_creator = create_fname(args.image_dir, "")
    train_df["image_id"] = train_df["image_id"].apply(filepath_creator)
    if mode == "train":
        test_df["image_id"] = test_df["image_id"].apply(filepath_creator)
...
def prepare_train_dataloader():
    ...
    if args.distributed:
        train_sampler = DistributedSampler(
            train_ds, num_replicas=args.world_size, rank=args.local_rank
        )
...
def batch(iterable, n=1):
    ...
    for ndx in range(0, l, n):
        yield np.array(
            [
                trans(Image.open(path).convert("RGB")).numpy()
                for path in iterable[ndx : min(ndx + n, l)]
            ],
            dtype=np.float32,
        )
