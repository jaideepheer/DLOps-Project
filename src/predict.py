...
def onnx_infer(val_imgs):
    session = onnxruntime.InferenceSession(
        args.triton_dir + args.model_name + "_onnx/1/model.onnx"
    )
...
def trt_infer(val_imgs):
    trt_engine = TensorRTInfer(
        args.triton_dir + args.model_name + "_trt_fp32/1/model.plan", args.batch_size
    )
...
def trt_infer16(val_imgs):
    trt_engine = TensorRTInfer(
        args.triton_dir + args.model_name + "_trt_fp16/1/model.plan", args.batch_size
    )
...
def trt_infer8(val_imgs):
    trt_engine = TensorRTInfer(
        args.triton_dir + args.model_name + "_trt_int8/1/model.plan", args.batch_size
    )
...
def infer(mode=args.mode):
    ...
    val_imgs = np.array(
        [trans(Image.open(path).convert("RGB")).numpy() for path in val_img_paths],
        dtype=np.float32,
    )

    start = time.time()
    if mode == "pytorch":
        rt = pytorch_infer(val_imgs)
    elif mode == "onnx":
        rt = onnx_infer(val_imgs)
    elif mode == "tensorrt32":
        rt = trt_infer(val_imgs)
    elif mode == "tensorrt16":
        rt = trt_infer16(val_imgs)
    elif mode == "tensorrt8":
        rt = trt_infer8(val_imgs)
    elapsed = time.time() - start
    if args.wandb:
        wandb.log({"total_infer_time": elapsed})
    return rt
...
def main():
    if args.wandb:
        wandb.init(
            ...
            config=args,
            entity="dlops-course",
            tags=["inference", args.model_name, args.mode],
        )
    ...
    pickle.dump(
        ...
        open(
            "{}/{}_pred.pkl".format(
                "/".join(args.out_weight.split("/")[:-1]), args.mode
            ),
            "wb",
        ),
    )
    fig_ = sns.heatmap(
        confusion_matrix(gt, pred), annot=True, cmap="Spectral", fmt="g"
    ).get_figure()
    fname = "{}/{}_confusion_matrix.jpg".format(
        "/".join(args.out_weight.split("/")[:-1]), args.mode
    )
    plt.savefig(fname)
    if args.wandb:
        wandb.log({"confusion_matrix": wandb.Image(fname)})
