...
def infer(mode = args.mode):
    ...
    val_imgs = np.array([trans(Image.open(path).convert('RGB')).numpy() for path in val_img_paths] , dtype=np.float32)
...
def main():
    if args.wandb:
        wandb.init(
            ...
            config=args, entity='dlops-course',
    ...
    fname = '{}/{}_confusion_matrix.jpg'.format('/'.join(args.out_weight.split('/')[:-1]), args.mode)
    plt.savefig(fname)
    if args.wandb:
